...
OcppMessage *ConnectorMeterValuesRecorder::loop() {
    ...
    if (connector && connector->getTransactionId() != lastTransactionId) {
        ...
        // Serial.println("Inside transaction clear");
        clear();
...
OcppMessage *ConnectorMeterValuesRecorder::toMeterValues() {
    int tran1 = getTransactionId();
    if(tran == -1)
    {
        // Serial.println("inside tran condition");
        clear();
        auto result = new MeterValues(&sampleTimestamp, nullptr , nullptr, connectorId, lastTransactionId);
        tran = tran1;
        return result;
    }
